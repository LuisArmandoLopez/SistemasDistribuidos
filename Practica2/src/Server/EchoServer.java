...
public class EchoServer {
    ...
    private static String tar=new String();
       ...
       // Servidor esta ahora a la escucha ( esperando conexiones)
        try {
            ...
            while(listening) // bucle infinito
            {
                ...
                System.out.println(myURL+": Estoy atendiendo un cliente");
                ...
                {    
                    String[] newStr = inputline.split("\\s+");
                    can=Integer.parseInt(newStr[0]);
                    tar=newStr[1];
                    cvv=Integer.parseInt(newStr[2]);
                    
                    System.out.println("\n"+myURL+": Recibi los siguientes datos:");
                    System.out.println("Tarjeta: "+tar);
                    System.out.println("cvv: "+cvv);
                    System.out.println("Monto: "+can);
                    
                    os.println(eo.echo(can,tar,cvv));//escribimos en el socket lo recibido mas mensaje
                    os.flush();//limpiamos el socket
                }
                System.out.println("\n"+myURL+"Servidor: termine de atender a un cliente");
                os.close();
                is.close();
                clientSocket.close();
            }
